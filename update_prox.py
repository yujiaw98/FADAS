...
class LocalUpdate(object):
    ...
    def update_weights_local(self, model, global_round, lrs):
        ...
        for p in model.parameters():
            global_model_param.append(p.data.detach().clone())

        model.train()
        epoch_loss = []

        optimizer = torch.optim.SGD(model.parameters(), lr=lrs[global_round], momentum=0.9, weight_decay=1e-4)
        # stepLR = StepLR(optimizer, step_size=self.args.local_lr, gamma=0.0)

        
        # for iters in range(self.args.local_ep):
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                proximal_term = 0.0
                for w, w_t in zip(model.parameters(), global_model_param):
                    proximal_term += ((w - w_t).norm(2))**2
                loss = loss1 + (0.001/ 2) * proximal_term
                loss.backward()
                optimizer.step()
...
#         return model.state_dict(), par_after, sum(epoch_loss) / len(epoch_loss)
    ...
    def update_weights(self, model, global_round):
        ...
        optimizer = torch.optim.SGD(par_after, lr=self.args.local_lr, momentum=0)
...
def update_model_inplace(model, par_before, delta, args, cur_iter, momentum_buffer_list, exp_avgs, exp_avg_sqs, max_exp_avg_sqs):
    ...
    for i, param in enumerate(model.parameters()): 
        ...
        # adam calculation
            ...
            denom = ((max_exp_avg_sqs[i]/bias_correction2).add_(args.eps)).sqrt()

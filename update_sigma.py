...
class LocalUpdate(object):
    ...
    def train_val_test(self, dataset, idxs):
        ...
        validloader = DataLoader(DatasetSplit(dataset, idxs_train),
                                 batch_size=int(len(idxs)), shuffle=True)
    ...
    def update_weights_local(self, model, global_round):
        ...
        model.train()
        epoch_loss = []
        optimizer = torch.optim.SGD(model.parameters(), lr=self.args.local_lr, momentum=0.9, weight_decay=1e-4)
        ...
        for batch_idx, (images, labels) in enumerate(self.validloader):
            ...
            for p in model.parameters():
                if p.grad is not None:   # Check if the gradient exists
                    fgrad.append(p.grad.detach())
            # print(fgrad[-1])
            model.zero_grad()
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                for p in model.parameters():
                    if p.grad is not None:   # Check if the gradient exists
                        sgrad.append(p.grad.detach())
                grad_diff = utils.sub_params(fgrad, sgrad)
                grad_norm = utils.l2_norm(grad_diff)
        ...
        return model.state_dict(), par_after, sum(epoch_loss) / len(epoch_loss), fgrad

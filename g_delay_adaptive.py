...
#     out_dir_name = args.model + args.dataset + args.optimizer + '_lr' + str(args.lr) + '_locallr' + str(args.local_lr) + '_localep' + str(args.local_ep) +'_localbs' + str(args.local_bs) + '_eps' + str(args.eps)
    file_name = '/asynfwdgda_{}_{}_{}_llr[{}]_glr[{}]_eps[{}]_le[{}]_bs[{}]_iid[{}]_mi[{}]_frac[{}]_update[{}]_scale[{}]_dir_alpha[{}]_{}.pkl'.\
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        ## global delay-adaptive learning rate
        if max(delay)>args.tauc:
            lr_lambda = 1/(max(delay))
        else:
            lr_lambda = 1

        update_model_inplace(global_model, par_before, global_delta, args, epoch, 
                momentum_buffer_list, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, lr_lambda)
